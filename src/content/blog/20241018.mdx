...
import {
  ...
  SAMPEL_2,
  SAMPEL_3,
} from '@/constants/content/blog/20241018.ts';
...
## 패키지 선택
![diagram](/images/blog/20241018/package-comparison.png)
[패키지 비교 링크](https://npmtrends.com/@tanstack/react-virtual-vs-react-virtualized-vs-react-virtuoso-vs-react-window)
여러가지 조건으로 두고봤을 때 
1. 업데이트가 지속적으로 이뤄진다.
2. 현재 표현되어있는 반응형과 관련해서 자동으로 지원해주는 부분이 있음.
3. 패키지 사이즈가 작다.**(253KB)**
4. 신뢰할만한 기업에서 해당 패키지를 비슷한 UI에서 적용한 사례가 있음.

위와 같은 이유로 react-virtuoso를 선택했습니다.

❗️사실 처음에는 @tanstack/react-virtual을 사용했지만, 반응형 UI에 대한 설명 및 지원이 친절하지 않은 것 같아 virtuoso의 공식문서를 보고 virtuoso로 변경했습니다.

## 마크업 개선사항
가상화 작업을 진행하기 전에 현재 UI의 html 구조 및 css 수정이 필요했다.
flex의 wrap과 basis/grow/shrink를 적절히 사용하여 구현이 되어있던 기존 UI를 이것을 Virtualization 패키지가 Row에 대한 값을 산출할 수 있게 변경해야했다. 따라서 useWindowSize라는 커스텀 훅을 사용해서 현재 뷰포트를 고려해서 백엔드에서 받아온 아이템을 아래와 같은 구조로 재배치하는 코드를 작성했다.
...
<Code code={SAMPEL_1} lang="typescript" />

## Virtuoso 적용 이후

<Code code={SAMPEL_2} lang="typescript" />
...
<Code code={SAMPEL_3} lang="typescript" />

## 개선사항

- article에서 div로 태그 변경되었습니다. 
...
- [\[Virtualized List\] \#2. Virtualized List의 기능](https://hini7.tistory.com/259)
